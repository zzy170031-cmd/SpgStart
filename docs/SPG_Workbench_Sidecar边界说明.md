...
这份文档用于说明当前 SPG 主工程里，`Workbench / AutoReason / Hermes 接力改造` 的正式边界，避免后续开发再次把 sidecar 试验层误当成核心主链路。
...
如果 sidecar 需要影响这些区域，必须经过主线重构决策，而不是以实验性增强名义直接侵入。

## 当前主线态度

- `AutoReason` 默认为关闭或 shadow 可观测
- 它可以记录额外判断、辅助候选 refinement、补充 workbench 观察数据
- 它不能直接成为新的唯一真值来源
- 当 sidecar 结果与主链路冲突时，以主链路稳定运行和可解释输出为先

## 迁移到新仓库后的执行原则

新仓库中，sidecar 相关改动继续允许存在，但必须遵守下面四条：

1. 只在 `spg-web/workbench` 边界内活动
2. 任何越界到 `spg-core` 的语义改动，都必须单独立项
3. 不能因为 sidecar 实验破坏桌面启动器、第二页 API 配置页、第三页主系统主链路
4. 每次更新后仍然要经过四代理机制复盘，再决定是否保留
