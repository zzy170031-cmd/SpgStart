...
function setHint(text, tone = "info") {
  const node = $("bootstrap-hint");
  if (!node) return;
  node.textContent = text;
  node.className = `hero-hint ${tone}`;
}
...
function render(page, gateState = page.gate_state) {
  ...
  setHint(
    gateState.local_user_confirmed
      ? gateState.api_gate_passed
        ? "这台设备已经完成本机确认和 API 校验。点击按钮后，会在当前窗口直接进入主系统。"
        : "这台设备已经完成本机确认。点击按钮后，会在当前窗口进入第二页 Provider 配置。"
      : "请先确认当前 Windows 用户。确认完成后，当前窗口会直接进入第二页 Provider 配置。",
    "info"
  );
}
...
async function run() {
  ...
  $("confirm-identity").addEventListener("click", async () => {
    ...
    try {
      ...
      setHint(`确认失败：${error.message || error}`, "error");
    } finally {
  ...
  $("reset-profile").addEventListener("click", async () => {
    try {
      ...
      render(page, gateState);
      setHint("已经清空当前机器的确认状态。你可以重新确认本机身份。", "success");
    } catch (error) {
      setHint(`重置失败：${error.message || error}`, "error");
    }
  });
}

run().catch((error) => {
  setHint(`初始化失败：${String(error)}`, "error");
});
