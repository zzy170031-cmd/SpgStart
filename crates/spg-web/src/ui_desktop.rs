...
<html lang="zh-CN">
  ...
  <body>
    ...
    <main class="shell">
      <header class="hero">
        <div>
          ...
          <h1>桌面入口确认</h1>
          <p class="subtitle">
            第一步只确认当前 Windows 用户。确认后，当前窗口会进入第二页 Provider 配置；
            如果当前机器已经完成 API 校验，也会从这里直接继续进入主系统。
          </p>
          <div id="bootstrap-hint" class="hero-hint">正在读取本机身份和启动状态...</div>
        </div>
        <div class="hero-meta">
          <div class="metric">
            <span>当前版本</span>
    ...
    <script type="module" src="/assets/desktop_launcher.js?v={version}"></script>
  </body>
</html>"#,
        state = state,
