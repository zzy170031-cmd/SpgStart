...
const ASSET_VERSION: &str = concat!(env!("CARGO_PKG_VERSION"), "-desktop-launcher-v5");
...
<html lang="zh-CN">
  ...
  <body class="desktop-launcher-page">
    <div id="desktop-launcher-root" data-state="{state}" hidden></div>
    <main class="shell launcher-shell">
      <section class="launcher-stack">
        <section class="panel launcher-card launcher-card-title">
          <p class="eyebrow">SPG / Windows Desktop</p>
          <h1>桌面入口确认</h1>
        </section>

        <section class="panel launcher-card launcher-card-user">
          <p class="panel-kicker">当前用户</p>
          <h2>当前 Windows 用户</h2>
          <div class="info-list">
        ...
        <section class="panel launcher-card launcher-card-meta">
          <p class="panel-kicker">版本号与当前阶段</p>
          <div class="launcher-meta-grid">
